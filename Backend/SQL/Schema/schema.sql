...
CREATE TABLE IF NOT EXISTS `HemoRS`.`doacao` (
  ...
  `data_e_hora_agendada` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `status` ENUM('agendada', 'triagem', 'cancelada', 'coletada', 'recusada') NULL DEFAULT 'agendada',
  `peso` INT NULL,
  `hemoglobina` INT NULL,
  `motivo_da_recusa` VARCHAR(255) NULL,
  `volume_coletado` INT NULL,
  `coletado_em` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
